library(tidyverse)
load("../../airbnb-crime_criminology_2024/data/output/dpm_quarter_fit.RData")
load("../../airbnb-crime_criminology_2024/data/output/dpm_quarter_fit_unstd.RData")
...
load("../../airbnb-crime_criminology_2024/data/output/dpm_half_ce_fit.RData")
plot_data <- dpm_half_ce_fit |>
  select(dv, spec, term, estimate, std.error, conf.low, conf.high) %>%
  mutate(term = ifelse(str_detect(term, "dp|dlg") & str_detect(term, "t - 1"), "crime (t - 1)", term),
         term = str_replace(term, "std_collective_efficacy", "Collective\nEfficacy"),
         across(c(dv, term), ~ str_to_title(str_replace_all(str_remove_all(., "(log_)|(std_)|(dlg_|dp_|abnb_)|(_aw$)?"), "_", " "))),
...
out_plot <- ggplot(plot_data, aes(x = estimate, y = dv)) +
  geom_tile(aes(alpha = graybar),
            width = Inf, height = 1, fill = "black") +
  scale_alpha_manual(values = c("black" = 0.3, "white" = 0)) +
  ylab(NULL) + xlab("Half yearly crimes per active rental (fully standardized)") +
  geom_point() +
  facet_grid( ~ term, scales = scales) +
  coord_cartesian(xlim = c(-0.10, 0.15), expand = TRUE) +
  geom_errorbarh(aes(xmin = conf.low, xmax = conf.high), height = 0.2, size = 0.2) +
  geom_vline(xintercept = 0, linetype = "dashed") +
  theme_minimal() +
  theme(text = element_text(family = "serif"),
        ...
        axis.title.y = ggtext::element_markdown(),
        legend.position = "none")
ggsave(plot = out_plot, filename = "./img/half-year-results.png", width = 4.5, height = 4.5)
